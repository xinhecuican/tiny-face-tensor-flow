...
class WIDERFace(dataset.Dataset):
    ...
    def __init__(self, path, templates, enable_edge=False, img_transforms=None, dataset_root="", split="train",
    ...
    def process_inputs(self, image, bboxes):
        ...
        edge = 0
        if self.enable_edge:
            edge = sobel(img, thres, bboxes=bboxes)
        return img, class_maps, regress_maps, bboxes, edge
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            ...
            img, class_map, reg_map, bboxes, edge = self.process_inputs(image, bboxes)
