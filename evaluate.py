...
def dataloader(args):
    ...
    val_loader, templates = get_dataloader(args.dataset, args,
                                           ...
                                           img_transforms=val_transforms, enable_edge=args.enable_edge)
    return val_loader, templates


def get_model(args, checkpoint=None, num_templates=25):
    model = DetectionModel(num_templates=num_templates, enable_edge=args.enable_edge)
    if checkpoint:
...
def run(args, model, val_loader, templates, prob_thresh, nms_thresh, device, split,
    ...
    for idx, (img, filename) in tqdm(enumerate(val_loader), total=len(val_loader)):
        if idx == 3005:
            dets = trainer.get_detections(model, img, templates, val_loader.dataset.rf,
                                          ...
                                          nms_thresh, device=torch.device('cpu'), enable_edge=args.enable_edge)
            continue
        dets = trainer.get_detections(model, img, templates, val_loader.dataset.rf,
                                      val_loader.dataset.transforms, prob_thresh,
                                      nms_thresh, device=device, enable_edge=args.enable_edge)
...
def main():
    ...
    model = get_model(args, args.checkpoint, num_templates=num_templates)

    with torch.no_grad():
        # run model on val/test set and generate results files
        run(args, model, val_loader, templates, args.prob_thresh, args.nms_thresh,
            device, args.split,
